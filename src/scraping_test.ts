...
const getPostsByPage = async (context: playwright.BrowserContext, pageNum = 1) => {
    const p = await context.newPage();
    await p.goto(`https://girlschannel.net/topics/${TOPICS_ID}/${ pageNum !== 1 ? pageNum : '' }`)

    const threadItems = await p.$$(".topic-comment .comment-item .body");
    const list = await Promise.all(threadItems.map(async (item) => await item.innerText()))
...
(async () => {
    ...
    const pageNum = await Promise.all(pager.map(async (item) => await item.innerText()))
    const set = [...new Set(pageNum)].filter((v) => v).map((v) => parseInt(v))

    const test = await Promise.all(Array.from(set).map(async (_pageNum) => {
    ...
    fs.writeFileSync(`./${TOPICS_ID}_sample.csv`, csvData);
